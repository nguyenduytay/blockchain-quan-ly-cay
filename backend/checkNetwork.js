...
try {
    const chaincodeOutput = execSync('docker ps | grep qlcaytrong', { encoding: 'utf8' });
    console.log('✅ Chaincode containers đang chạy:');
    console.log(chaincodeOutput);
} catch (error) {
    console.error('❌ Không tìm thấy chaincode containers!');
    console.error('   Chạy: cd /fabric-samples/test-network && ./network.sh deployCC -ccn qlcaytrong -ccp ../chaincode/qlcaytrong/javascript -ccl javascript');
}
