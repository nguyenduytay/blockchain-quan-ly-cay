...
try {
    const chaincodeOutput = execSync('docker ps | grep thuoctay', { encoding: 'utf8' });
    console.log('✅ Chaincode containers đang chạy:');
    console.log(chaincodeOutput);
} catch (error) {
    console.error('❌ Không tìm thấy chaincode containers!');
    console.error('   Chạy: cd /fabric-samples/test-network && ./network.sh deployCC -ccn thuoctay -ccp ../chaincode/thuoctay/javascript -ccl javascript');
}
