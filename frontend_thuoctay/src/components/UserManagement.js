      ...
      <Card className="mb-4">
        <Card.Body>
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <Button variant="danger" onClick={fetchUsers} disabled={loading} style={{ background: 'linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)', border: 'none' }}>
              Làm mới
            </Button>
          </div>
                    ...
                    users.map((item) => {
                      ...
                      return (
                        <tr key={record.username}>
                          <td><Badge bg="danger">{record.username}</Badge></td>
                          <td>{record.fullName}</td>
                          <td>{record.email}</td>
                          <td>{record.phone || '-'}</td>
                          <td>
                            <Badge bg={record.role === 'admin' ? 'danger' : record.role === 'manager' ? 'warning' : 'info'}>
                              {record.role === 'admin' ? 'Admin' : record.role === 'manager' ? 'Manager' : 'User'}
                            </Badge>
                          </td>
                          ...
                          <td>
                            <div className="d-flex gap-1">
                              <Button size="sm" variant="warning" onClick={() => handleOpenModal(item)}>
                                Sửa
                              </Button>
                              {record.username !== currentUser.username && currentUser.role === 'admin' && (
                                <Button 
                                  size="sm" 
                                  variant="danger" 
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Footer>
            ...
            <Button variant="danger" type="submit" style={{ background: 'linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)', border: 'none' }}>Lưu</Button>
          </Modal.Footer>
        </Form>
      </Modal>
